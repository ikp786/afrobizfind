...
<body>
		...
		@foreach($product as $prod)
			@if(!in_array($prod->orderno, $unique_order))
				...
				<tr>
					...
					<th align="left" class="tbl-th">Order status</th>
				</tr>
				<tr>
					<td align="left" class="tbl-td">{{ $prod->orderno }}</td>
					<td align="left" class="tbl-td">{{ $prod->orderstatus }}</td>
				</tr>
				<tr>
	...
	<table width="100%" border="0" class="tbl-bg">
		<tr>
			...
			?>
			@endforeach
			<th colspan="3" class="tbl-head">Total price: {{ $match[1] }} {{ $total }}</th>
		</tr>
	</table>
